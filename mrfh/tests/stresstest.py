#!/usr/bin/env python
""" stresstest.py:  A stress-tester for MultiprocessRotatingFileHandler
...
class RotateLogStressTester:
    ...
    def getLogHandler(self, fn):
        ...
        class. """
        return MultiprocessRotatingFileHandler(fn, 'a', self.rotateSize,
                                             self.rotateCount)
...
parser = OptionParser(usage="usage:  %prog",
                      description="Stress test the MRFH module.")
...
def main_runner(args):
    ...
    empty = unified_diff(client_combo, shared_combo)
    if empty:
        print("Passed! :)")
        sys.exit(0)
    else:
